...
class wrenchActionGrammar:
    ...
    def __init__(self,ss,fs='SIM_HIRO_ONE_SA_ERROR_CHARAC_Prob',armSide='right'):
        ...
        if str(armSide).lower() == 'left':
            self.leftArm=1
        if str(armSide).lower() == 'right':
            self.rightArm=1
        ...
        # jaccard
        self.hlb_pickle                  = 'allTrials_hlbStates.pickle'
        self.state_probabilities_pickle  = 'state_prob_onelevel.pickle'
        self.results_pickle              = 'results.pickle'

        #------------------------------------------------------------------------------
        # lcss
        self.trial_lcss_mat_pickle         = 'trial_lcss_mat.pickle'
        self.sim_state_metric_pickle       = 'sim_state_metric.pickle'
        self.lcss_results_pickle           = 'lcss_results.pickle'
        self.permutation_matrix_pickle     = 'permutation_matrix.pickle'
        self.classification_matrix_pickle  = 'classification_matrix.pickle'
        self.avg_prob_vector_pickle        = 'avg_prob_vector.pickle'
        self.acuracy_numTrials_pickle      = 'accuracy_numTrials.pickle'
        ...
        self.armSide_dir=os.path.join(self.exp_dir,armSide.lower())

        # allTrialLabels path to pickle file
        self.allTrialLabels_path=os.path.join(self.armSide_dir,self.hlb_pickle)
...
#        allTrialLabels={}
    ...
    #--------------------------------------------------------------------------
        ...
        self.trial_lcss_mat_pickle_path=os.path.join(self.classification_dir,'trial_lcss_mat.pickle')
        ...
        #                self.lcss_trials_mat=pickle.load(handle)
            ...
            # IF there is no data (not loaded from file) then create self.lcss_trials_mat
            if not bool(self.lcss_trials_mat[0][0][0][0]):
                if os.path.isfile( self.trial_lcss_mat_pickle_path ):
                    if self.loadFromFileFlag:
                        ...
                        with open( trial_lcss_mat_pickle_path,'wb') as handle:
            ...
            with open(self.classification_dir + '/' + self.permutation_matrix_pickle, 'wb') as handle:
            ...
            self.fd = open(os.path.join( self.classification_dir,self.armSide_dir,self.permutation_matrix_pickle[:-7]+'.txt'), "wb")
            ...
            #pickle
            with open(self.classification_dir + '/' + self.sim_state_metric_pickle, 'wb') as handle:
                pickle.dump(self.sim_state_metric, handle, protocol=pickle.HIGHEST_PROTOCOL)

            # Save binary numpy to file (maybe more efficient than pickle and can be opened with np.loadtxt)
            fd = open(os.path.join(self.classification_dir,self.sim_state_metric_pickle[:-7]+'.txt'), "wb")
            np.save(self.fd,self.sim_state_metric)
            self.fd.close()
            ...
            #pickle
            with open(self.classification_dir + '/' + self.classification_matrix_pickle, 'wb') as handle:
                pickle.dump(self.sim_state_metric, handle, protocol=pickle.HIGHEST_PROTOCOL)

            # Save binary numpy to file (maybe more efficient than pickle and can be opened with np.loadtxt)
            self.fd = open(os.path.join(self.classification_dir,self.classification_matrix_pickle[:-7]+'.txt'), "wb")
            np.save(self.fd,self.sim_state_metric)
            self.fd.close()
...
#            self.fd.close()
            ...
            # Save results
            #pickle
            with open( self.classification_dir + '/' + self.lcss_results_pickle, 'wb') as handle:
            ...
            with file(os.path.join( self.classification_dir, self.lcss_results_pickle[:-7]+'.txt'), 'w') as outfile:
                outfile.write('# Array shape: {0}\n'.format(self.res.shape))
                for data_slice in self.res:
                    np.savetxt(self.outfile, self.data_slice, fmt='%0.3f')
                    self.outfile.write('# New slice\n')

        np.savetxt( self.classification_dir + '/' + self.acuracy_numTrials_pickle, self.accuracy_numTrials,'%0.2f')
...
if __name__== '__main__':
    try:
        ...
        levels=['llbehavior'] # what rcbht level do you want to check? Provide the name of one or more.
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
        ...
        # For two arms
        # Local Declarations
        res                  =np.zeros( (rWAG.l_len,       rWAG.s_len)       )
        accuracy_numTrials   =np.zeros( (rWAG.numTrials-1, rWAG.results_dim) )

        # Compute dual arm accuracy results
        for ll in range(rWAG.l_len):
            for ctr in range(rWAG.s_len):

                res[ll]=np.multiply( r_classification_vec[ll].diagonal(), l_classification_vec[ll].diagonal() )
            print 'Dual arm accuracy for level: ' + ll+1 + ' is: ' + res.prod() + '\n'

        for kk in xrange(2,rWAG.numTrials):
            accuracy_numTrials[kk-1]=np.array( [kk-1, res[0][0], res[0][1], res[0][2], res[0][3], res.prod()] )
        np.savetxt( rWAG.classification_dir + '/' + 'two_arm_' + rWAG.acuracy_numTrials_pickle, accuracy_numTrials, '%0.2f' )

        # Plot
        import matplotlib.pyplot as plt
        plt.plot(rWAG.accuracy_numTrials[:,0], rWAG.accuracy_numTrials[:,5])
        plt.show()
