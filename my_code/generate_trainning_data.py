...
avg_prob_vector_pickle        = 'avg_prob_vector.pickle'
acuracy_numTrials_pickle      = 'accuracy_numTrials.pickle'
...
if successFlag:
    ...
    for data_folder_name in data_folder_names:
        ...
        dict_cooked_from_folder = data_folder_parser.parse_folder(data_folder_full_path,armFlag,split_by_states=False)
...
if hlStatesFlag:
    ...
    for data_folder_name in data_folder_names:
        ...
        try:
            dict_cooked_from_folder = data_folder_parser.parse_folder(data_folder_full_path, armFlag, split_by_states=True)
            if dict_cooked_from_folder == None:
                continue
            else:
                dict_all[data_folder_name]=dict_cooked_from_folder
...
#--------------------------------------------------------------------------
    ...
    results_dim=s_len+2 # The extra two numbers are time and the product of each of the states accuracy
    accuracy_numTrials   =np.zeros( (numTrials-1,results_dim) ) #-1 trial because it is used for validation in "leave-one-out" cross validation.
    for kfold in xrange(1,numTrials):
    # Crossfold training and testing generator
        kf=cross_validation.LeaveOneOut(numTrials)
    ...
    #                lcss_trials_mat=pickle.load(handle)  
        ...
        #   similarity, then it is deemed to have been selected. 
        ...
        for l in range(l_len):
            for ctr in range(s_len):
                res[l]=classification_vector[l].diagonal()
                print 'For level: [',l+1,'/',l_len,'] and State [',ctr+1,'/',s_len,'] accuracy is: ',res[l][ctr],'\n'
            print 'Task accuracy for level: ',l+1, ' is: ',res.prod(),'\n'
        print str(kfold) + ' ' + str(res[0][0]) + ' ' + str(res[0][1]) + ' ' + str(res[0][2]) + ' ' + str(res[0][3]) + ' ' + str(res.prod())
        accuracy_numTrials[kfold-1]=np.array( [kfold, res[0][0], res[0][1], res[0][2], res[0][3], res.prod()] )
        # Save results
        #pickle
    ...
    np.savetxt(classification_dir+'/'+acuracy_numTrials_pickle, accuracy_numTrials,'%0.2f')
            
    # Plot
    import matplotlib.pyplot as plt
    plt.plot(accuracy_numTrials[:,0], accuracy_numTrials[:,4])
    plt.show()
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
